...
print(
    """clock.py - clock segment example.
...
fmt = "%M"
address = 0x61
available = {"hour": "%H", "minute": "%M", "second": "%S"}
...
try:
    ...
    raise OSError(
        "Unable to find LTP305 on i2c address: 0x{:02x}. Run `i2c-detect -y 1` to discover breakouts".format(
            address
        )
    )
